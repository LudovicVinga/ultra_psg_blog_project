...
final class CategoryController extends AbstractController
{
    ...
    public function create(Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash('success', 'La catégorie a bien été créee.');
    ...
    // On récupère la catégorie a modifier via le param converter
    ...
    public function delete(Category $category, Request $request, EntityManagerInterface $entityManager): Response
    {
        if ($this->isCsrfTokenValid("delete-category-{$category->getId()}", $request->request->get('csrf_token'))) {
            ...
            $this->addFlash('success', "La catégorie '{$categoryName}' ainsi que les articles associés ont bien été supprimés.");
        }
