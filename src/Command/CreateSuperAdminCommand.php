...
#[AsCommand(
    ...
    description: 'This command creates the super administrator',
)]
class CreateSuperAdminCommand extends Command
{
    ...
    protected function interact(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!empty($plainPassword)) {
            $this->io->warning('Le mot de passe ne doit pas être affiché en clair dans le terminal, veuillez renseinger cette valeur à nouveau');
        }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($users as $user) {
            if (in_array('ROLE_SUPER_ADMIN', $user->getRoles())) {
                $this->io->error('Problem: Le super admin éxiste déjà.');
